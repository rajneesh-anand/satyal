...
export const SelectPlan = ({
  ...
  useEffect(() => {
    ...
    const fetchPricingData = async () => {
      ...
      let filter_pricings=result.filter((item:pricing_plans)=>item?.plan_Classes.includes(studentClass?.value))
            ...
            >
              <div className="flex-1">
                <h3 className={cn(
                  ...
                  item.plan_name === selectedPlan?.plan_name
                  ?"text-white"
                  :"text-gray-900")}>{item.plan_name}</span>
                 </h3>
                {item.most_popular ? (
                ...
                {item.plan_discounted_fee > 0 ? (
                  <p className={cn(
                    ...
                    item.plan_name===selectedPlan?.plan_name?"text-white":"text-gray-900")}>
                  ...
                  <p className={cn(
                    "my-1 md:my-2 flex justify-center items-baseline group-hover:text-white",
                    item.plan_name===selectedPlan?.plan_name?"text-white":"text-gray-900")}>
                      <span className="ml-1 text-xl md:text-2xl font-bold tracking-tight">{item.plan_fee} NPR</span>
                   
                  </p>
                )}
                <p className={cn("mt-1 md:mt-2 group-hover:text-white",
                item.plan_name===selectedPlan?.plan_name?"text-white":"text-gray-500")}>{item.plan_description}</p>

                <ul role="list" className="mt-6 space-y-1.5">
                  {item.features.map((feature, index) => (
                    <li key={index} className="flex items-center">
                      <AiOutlineCheck
                        className={cn("h-4 w-4 flex-shrink-0  group-hover:text-white",item.plan_name===selectedPlan?.plan_name?"text-white":"text-indigo-500")}
                        aria-hidden="true"
                      />
                      <span className={cn("ml-2 text-sm md:text-md group-hover:text-white",item.plan_name===selectedPlan?.plan_name?"text-white":"text-gray-500")}>{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
